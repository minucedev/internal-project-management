...
import { MemberTable, InviteMemberModal, TaskBoard } from '../components';
    ...
    return (
        <div>
            ...
            <Tabs tabs={tabs} activeKey={activeTab} onChange={setActiveTab}>
                {/* Board tab — TaskBoard Sprint 1 */}
                {activeTab === 'board' && (
                    <div className="bg-white rounded-xl shadow-md p-6 overflow-x-auto">
                        <TaskBoard
                            projectId={id!}
                            role={project?.currentUserRole ?? 'VIEWER'}
                            currentUserId={project?.createdById ?? 0}
                            members={memberList?.members ?? project?.members ?? []}
                        />
                    </div>
                )}
